...
KuaiYongIosImp::KuaiYongIosImp() : isKyLogin(false), kyToken("")
{
    
}

KuaiYongIosImp::~KuaiYongIosImp()
{
    
}

void KuaiYongIosImp::setUIWindow(UIWindow *window)
{
    win = window;
}

//go2play的sdk启动
void KuaiYongIosImp::sdkStart()
{
    [[KYSDK instance] setSdkdelegate:[[KuaiYongIosDeg alloc] init]];
}



//oc 向go2play 请求注册消息
void KuaiYongIosImp::sdkRegister(UserAndExtInfo &uei)
{
...
//oc 向go2play 请求登陆消息
void KuaiYongIosImp::sdkLogin(UserAndExtInfo &uei)
{
//    //从输入框中获取用户名和密码
//    NSString *uName = [NSString stringWithUTF8String:uei.userName.c_str()];
//    NSString *uPw = [NSString stringWithUTF8String:uei.userPwd.c_str()];
...
//快速登陆实现
void KuaiYongIosImp::sdkFastloginGame(UserAndExtInfo &uei)
{
//    NSMutableDictionary *jsonDic = [NSMutableDictionary dictionary];
//
//    std::string uid = GameConfig::getUdit();
...
//普通登陆，本地已有帐户的情况
void KuaiYongIosImp::sdkLoginGame(UserAndExtInfo &uei)
{
//    NSMutableDictionary *jsonDic = [NSMutableDictionary dictionary];
//    
//    NSString *uName = [NSString stringWithUTF8String:uei.userName.c_str()];
...
//facebook登陆实现
void KuaiYongIosImp::sdkFBLogin(UserAndExtInfo &uei)
{
//    NSMutableDictionary *jsonDic = [NSMutableDictionary dictionary];
//    std::string uid = GameConfig::getUdit();
//    NSString *uuid = [NSString stringWithUTF8String:uid.c_str()];
...
//facebook 登出
void KuaiYongIosImp::sdkFBLogout()
{
//    NSMutableDictionary *jsonDic = [NSMutableDictionary dictionary];
//    //1.2 sdk新增参数传入。
//    [jsonDic setObject:@"facebook" forKey:GMF_PARAM_PROVIDER];
...
//加密字符串
std::string KuaiYongIosImp::sdkGetEncryptLoginKey(const std::string& lkStr)
{
...
//显示登录注册界面。
void KuaiYongIosImp::showUserView()
{
    [[KYSDK instance] showUserView];
}

//退出登录。
void KuaiYongIosImp::userLogOut()
{
    isKyLogin = false;
    kyToken = "";
...
//显示用户设置界面。
void KuaiYongIosImp::setUpUser()
{
    [[KYSDK instance] setUpUser];
}

//打开支付界面。
void KuaiYongIosImp::showPaymentView()
{
//    [KYSDK instance] showPayWith:(NSString *) fee:(NSString *) game:@"8135" gamesvr:<#(NSString *)#> subject:<#(NSString *)#> md5Key:<#(NSString *)#> userId:<#(NSString *)#> appScheme:<#(NSString *)#>;
}