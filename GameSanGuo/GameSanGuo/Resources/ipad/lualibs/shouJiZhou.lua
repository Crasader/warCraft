...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityShouJingZhou.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityShouJingZhouBg.png")
		spriteBg:setAnchorPoint(ccp(0.5,0.5))
        spriteBg:setPosition( visibleSize.width * 0.5 ,tempLayer:getPositionY() + tempLayer:getContentSize().height * 0.5 + spriteBg:getContentSize().height * 0.5 +15)
		...
		--第二个label  活动描述
		local labelActivityDes = SGCCLabelTTF:create("活动期间，守荆州层数" , FONT_PANGWA , 22)
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654356)
end
