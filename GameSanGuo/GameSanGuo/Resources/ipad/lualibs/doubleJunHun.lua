...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityDoubleJunHun.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityDoubleJunHunBg.png")
        ...
        labelTime:setPosition(spriteBg:getContentSize().width * 0.5-180 , spriteBg:getContentSize().height - 120)
        spriteBg:addChild(labelTime)

		local labelActivityTime = SGCCLabelTTF:create("2014/08/22--2014/08/22" , FONT_PANGWA , 20)
		labelActivityTime:setPosition(spriteBg:getContentSize().width * 0.5 + 60 , spriteBg:getContentSize().height - 120)
		labelActivityTime:setTag(baseTag)
		print("活动时间tag = "..baseTag)
		baseTag = baseTag + 1
		spriteBg:addChild(labelActivityTime)
		--第二个label  活动描述
        local labeldes = SGCCLabelTTF:create("活動說明" , FONT_PANGWA , 30,CCSizeMake(150,50),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter,ccc3(0,255,0),ccc3(0,0,0),2,true);
        labeldes:setPosition(spriteBg:getContentSize().width * 0.5 , 130)
        spriteBg:addChild(labeldes)

		local labelActivityDes = SGCCLabelTTF:create("活动期间，活動副本全部開放 活动期间，活動副本全部開放 活动期间，活動副本全部開放" , FONT_PANGWA , 22,CCSizeMake(450,60),kCCTextAlignmentCenter,kCCVerticalTextAlignmentCenter,ccc3(255,255,255),ccc3(0,0,0),2,true)
		labelActivityDes:setPosition(spriteBg:getContentSize().width * 0.5 , 70)
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654360)
end
