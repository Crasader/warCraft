...
local function main()
	...
	local function createTemplateLayer(count , baseTag, buttonTag)
		local ret = CCLayer:create()
	...
	local function createRetLayer(baseTag)
        ...
        --添加可获得物品的layer-4
		local tempLayer_4 = createTemplateLayer(4 , 101, 140)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activity_reward_sendpower.png")
		spriteBg:setAnchorPoint(ccp(0.5,0.5))
        
		retLayer:addChild(spriteBg)

		tempLayer_4:setPosition(visibleSize.width/2 , visibleSize.height - 350 -  tempLayer_4:getContentSize().height * 0.5 - 15 - spriteBg:getContentSize().height)
		spriteBg:setPosition( visibleSize.width * 0.5 - 10,tempLayer_4:getPositionY()  + tempLayer_4:getContentSize().height * 0.5  + spriteBg:getContentSize().height * 0.5 +15)
		...
		--第二个label  活动描述
		local labelActivityDes = SGCCLabelTTF:create("活动期间，每日赠送好友体力即可获得相应的" , FONT_PANGWA , 22)
        ...
        retLayer:setContentSize( CCSize:new_local( visibleSize.width ,  visibleSize.height - 350 + 20) )
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654352)
end
