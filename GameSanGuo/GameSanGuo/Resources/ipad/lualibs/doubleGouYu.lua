...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityDoubleGouYu.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityDoubleGouYuBg.png")
		...
		-- 添加前往按钮
		local gotoButton = createLayerBtn(88,"an.png","前往","_goto_special_battle",false)
		gotoButton:setPosition(spriteBg:getContentSize().width * 0.5 - gotoButton:getContentSize().width * 0.5 + 20  ,130)
        ...
        labelTime:setPosition(spriteBg:getContentSize().width * 0.5-185 , 70)
        spriteBg:addChild(labelTime)

		local labelActivityTime = SGCCLabelTTF:create("2014/08/22 15:16--2014/08/22 15:16" , FONT_PANGWA , 20)
		labelActivityTime:setPosition(spriteBg:getContentSize().width * 0.5 + 55 , 70)
		labelActivityTime:setTag(baseTag)
		print("活动时间tag = "..baseTag)
		baseTag = baseTag + 1
		spriteBg:addChild(labelActivityTime)
		--第二个label  活动描述
        local labeldes = SGCCLabelTTF:create("活動說明:" , FONT_PANGWA , 22,CCSizeMake(150,50),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter,ccc3(0,255,0),ccc3(0,0,0),2,true);
        labeldes:setPosition(spriteBg:getContentSize().width * 0.5-185 , 40)
		...
		local labelActivityDes = SGCCLabelTTF:create("在活动时间内，幻境掉落勾玉數量翻倍!" , FONT_PANGWA , 22)
		labelActivityDes:setPosition(spriteBg:getContentSize().width * 0.5 + 45 , 40)
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654363)
end
