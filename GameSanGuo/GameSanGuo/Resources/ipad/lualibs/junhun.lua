...
local function main()
    ...
    ResourceManager:sharedInstance():bindTexture("sanguobigpic/junhun.plist", 1, 191)
...
-------顶图
    local sprite2 = CCSprite:createWithSpriteFrameName("junhun.png")
    sprite2:setPosition(ccpAdd(ccp(spriteqp:getPosition()),ccp(-4,sprite2:getContentSize().height/2+spriteqp:getContentSize().height/2)))
    layerFarm:addChild(sprite2)
    
-------底框字
    local labelinfo = CCLabelTTF:create("全民獎勵:活動截止時,累計獲得軍魂高于           的主公可獲得",FONT_BOXINFO,20)
...
-----level
    ...
    labellevel:setPosition(ccpAdd(ccp(labelinfo:getPosition()),ccp(113,0)))
    layerFarm:addChild(labellevel)
    labellevel:setTag(404)
    
-----三个国家+军魂数
    local labelwei = CCLabelTTF:create("第1名:",FONT_BOXINFO,22)
    labelwei:setAnchorPoint(CCPoint(0,0.5))
    labelwei:setPosition(ccpAdd(ccp(sprite2:getPosition()),ccp(-70,-270)))
    ...
    local labeljh = CCLabelTTF:create("活動期間累計獲得軍魂:",FONT_BOXINFO,22)
    labeljh:setAnchorPoint(CCPoint(0,0.5))
    labeljh:setPosition(ccpAdd(ccp(labelwu:getPosition()),ccp(-40,-35)))
    layerFarm:addChild(labeljh)
    --labeljh:setColor(ccc3(255,0,0))
...
-----------三个国家玩家名字+军魂数
   ...
   local playerwu = CCLabelTTF:create(" 暫無",FONT_BOXINFO,22)
    ...
    local playerjh = CCLabelTTF:create("10000",FONT_BOXINFO,22)
    playerjh:setAnchorPoint(CCPoint(0,0.5))
    playerjh:setPosition(ccpAdd(ccp(labeljh:getPosition()),ccp(labeljh:getContentSize().width,0)))
    layerFarm:addChild(playerjh)
    --playerjh:setColor(ccc3(255,0,0))
    playerjh:setTag(403)

-------------天\时\分\秒
    local day = CCLabelTTF:create("0",FONT_BOXINFO,30)
    day:setPosition(ccpAdd(ccp(sprite2:getPosition()),ccp(-53,216)))
...
------------大按钮
    ...
    btn6:setPosition(ccpAdd(ccp(sprite2:getPosition()),ccp(4,-13)))
    btn6:setTag(300)
    layerFarm:addChild(btn6)

-----------第二三名
    local btn4 =  createLayerButton4()
    btn4:setPosition(ccpAdd(ccp(btn6:getPosition()),ccp(-190,-190)))
    btn4:setTag(301)
    layerFarm:addChild(btn4)
    
    local btn5 =  createLayerButton5()
    btn5:setPosition(ccpAdd(ccp(btn6:getPosition()),ccp(190,-190)))
    btn5:setTag(302)
    ...
    sceneGame:addChild(createLayerFarm(),1,654331)
