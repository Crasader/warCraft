...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityChuzhiFanli.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityChuzhiFanliBg.png")
		...
		-- 添加前往按钮
		local gotoButton = createLayerBtn(88,"an.png","前往","showStoreLayer",false)
		gotoButton:setPosition(spriteBg:getContentSize().width - 170,180)
        ...
        labelTime:setPosition(spriteBg:getContentSize().width * 0.5-185 , 130)
        spriteBg:addChild(labelTime)

		local labelActivityTime = SGCCLabelTTF:create("2014/08/22 15:16--2014/08/22 15:16" , FONT_PANGWA , 20)
		labelActivityTime:setPosition(spriteBg:getContentSize().width * 0.5 + 55 , 130)
        ...
        labeldes:setPosition(spriteBg:getContentSize().width * 0.5-185 , 100)
		...
		local labelActivityDes = SGCCLabelTTF:create("活动时间内,充值任意金額可獲等30%的金額返利!" , FONT_PANGWA , 20 ,CCSizeMake(400,70),kCCTextAlignmentLeft,kCCVerticalTextAlignmentCenter,ccc3(255,255,255),ccc3(0,0,0),2,true)
		labelActivityDes:setPosition(spriteBg:getContentSize().width * 0.5 + 45 , 85)
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654364)
end
