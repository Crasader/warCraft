...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityDoubleArenaCoin.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityDoubleArenaCoinBg.png")
		...
		-- 添加前往按钮
		local gotoButton = createLayerBtn(88,"an.png","前往","_goto_arena_home",false)
		gotoButton:setPosition(spriteBg:getContentSize().width - 170,130)
        ...
        labelTime:setPosition(spriteBg:getContentSize().width * 0.5-185 , 90)
        spriteBg:addChild(labelTime)

		local labelActivityTime = SGCCLabelTTF:create("2014/08/22 15:16--2014/08/22 15:16" , FONT_PANGWA , 20)
		labelActivityTime:setPosition(spriteBg:getContentSize().width * 0.5 + 55 , 90)
        ...
        labeldes:setPosition(spriteBg:getContentSize().width * 0.5-185 , 60)
		...
		local labelActivityDes = SGCCLabelTTF:create("活动时间内，竞技场排名征服点奖励翻倍!" , FONT_PANGWA , 22)
		labelActivityDes:setPosition(spriteBg:getContentSize().width * 0.5 + 45 , 60)
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654362)
end
