...
- (void)showActivityIndicator:(UIActivityIndicatorViewStyle)style
{
    ...
    {
        printf("[CCNative_objc] ERR, showActivityIndicator() activity indicator already visible");
        return;
    }
    
    printf("[CCNative_objc] showActivityIndicator()");
...
- (void)hideActivityIndicator
{
    if (!activityIndicatorView_)
    {
        printf("[CCNative_objc] ERR, hideActivityIndicator() activity indicator not visible");
        return;
    }
    
    printf("[CCNative_objc] hideActivityIndicator()");
   ...
   andCancelButtonTitle:(NSString *)cancelButtonTitle
{
    if (alertView_)
    {
        printf("[CCNative_objc] ERR, createAlertView() alert view already exists");
        return;
    }
    
    printf("[CCNative_objc] createAlertView() title: %s, message: %s, cancelButtonTitle: %s",
...
- (NSInteger)addAlertButton:(NSString *)buttonTitle
{
    if (!alertView_)
    {
        printf("[CCNative_objc] ERR, addAlertButton() alert view not exists");
        return 0;
    }

    printf("[CCNative_objc] addAlertButton() buttonTitle: %s", utf8cstr(buttonTitle));
...
- (void)showAlertViewWithDelegate:(CCAlertViewDelegate *)delegate
{
    if (!alertView_)
    {
        printf("[CCNative_objc] ERR, showAlertViewWithDelegate() alert view not exists");
        return;
    }

    printf("[CCNative_objc] showAlertViewWithDelegate()");
...
- (void)removeAlertView
{
    if (!alertView_)
    {
        printf("[CCNative_objc] ERR, removeAlertView() alert view not exists");
        return;
    }
    
    printf("[CCNative_objc] removeAlertView()");
    [alertView_ release];
    alertView_ = nil;
#if CC_LUA_ENGINE_ENABLED > 0
    [self removeAlertViewLuaListener];
...
- (void)cancelAlertView
{
    if (!alertView_)
    {
        printf("[CCNative_objc] ERR, cancelAlertView() alert view not exists");
        return;
    }
    
    printf("[CCNative_objc] cancelAlertView()");
...
- (void)showAlertViewWithLuaListener:(LUA_FUNCTION)listener
{
    if (!alertView_)
    {
        printf("[CCNative_objc] ERR, showAlertViewWithLuaListener() alert view not exists");
        return;
    }
    
    printf("[CCNative_objc] showAlertViewWithLuaListener()");
