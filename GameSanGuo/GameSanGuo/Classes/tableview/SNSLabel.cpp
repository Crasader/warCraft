...
bool SNSLabel::initWithString(const char* string, const char* fontName, float fontSize, ccColor3B fontColor, ccColor3B shadowColor, float shadowSize, CCSize size, CCTextAlignment align)
{
	if (!CCLayer::init()) {
		return false;
	}
	...
	if (size.width != 0 || size.height != 0) {
		m_label = CCLabelTTF::create(string, fontName, fontSize, size, align);
	}
	m_label->retain();
	...
	this->addChild(shadow, 0, 1);
	
	return true;
}

SNSLabel* SNSLabel::create(const char* string, const char* fontName, float fontSize, ccColor3B fontColor, ccColor3B shadowColor, float shadowSize, CCSize size, CCTextAlignment align)
{
	SNSLabel *instance = new SNSLabel();
	if (instance && instance->initWithString(string, fontName, fontSize, fontColor, shadowColor, shadowSize, size, align)) {
...
void SNSLabel::setAnchorPoint(CCPoint point)
{
	CCLayer::setAnchorPoint(point);
...
void SNSLabel::setDimensions(const CCSize &dim)
{
	this->removeAllChildrenWithCleanup(true);
...
//	label->setBlendFunc((ccBlendFunc){GL_ONE, GL_ONE});
