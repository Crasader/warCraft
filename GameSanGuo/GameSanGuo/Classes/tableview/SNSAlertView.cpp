...
SNSAlertStack::SNSAlertStack():m_nowObject(NULL), m_isRuning(false), m_maskLayer(NULL), m_singleObject(NULL), m_waitQueue(NULL)
{
...
void SNSAlertStack::show(SNSAlertView *obj)
{
	...
	switch (showType) {
		...
		{
			temp->setScale(0.001);
			CCScaleTo* scale = CCScaleTo::create(0.15f, 0.6f, 1.4f);
			CCScaleTo* scale2 = CCScaleTo::create(0.05f, 1.3f, 0.7f);
			CCScaleTo* scale3 = CCScaleTo::create(0.08f, 0.8f, 1.2f);
			CCScaleTo* scale4 = CCScaleTo::create(0.1f, 1.0f, 1.0f);
			
			seq = CCSequence::create(scale, scale2, scale3, scale4, NULL);
		}
			break;
		case SNSAlertViewShowTypeStrong:
		{
			temp->setScale(1.6f);
			CCScaleTo* scale = CCScaleTo::create(0.6f, 1.0f);
			CCEaseElasticOut* out = CCEaseElasticOut::create(scale);
