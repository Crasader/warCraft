...
class SNSTableView;
//#define ITEMACTIONTIME 0.05f
/// MT效果时间变更
#define ITEMACTIONTIME 0.1f
typedef enum {
...
typedef enum {
	...
	TableViewSelectTypeSingle,			// TableView选中item的类型：单选不可反选
	TableViewSelectTypeSingleOpposite,	// TableView选中item的类型：单选可反选
	TableViewSelectTypeMulti,			// TableView选中item的类型：多选
} TableViewSelectType;

typedef enum {
	TableViewActionTypeINVALID = 0,
	TableViewActionTypeRight,			// TableView选中item出现类型：从右至左
	TableViewActionTypeMiddleScale,	    // TableView选中item出现类型：中间拉伸
	TableViewActionTypeMultiMove,			// TableView选中item出现类型：穿插移动
} TableViewActionType;
...
public:
    ...
    static void showScrollItemScale(SNSTableView* tableView);
    static void showScrollItemMultiMove(SNSTableView* tableView);
	//10.29//george
	static void showScrollItemMultiMoveSlide(SNSTableView* tableView);
	static void showScrollItemMultiMoveSlideHide(SNSTableView* tableView);
	//11.26//kanata
	static void showScrollItemMultiMoveFromRight(SNSTableView* tableView);//加入果冻效果
    //所谓果冻效果 向量表示为  向左移动10个单位，在向右移动4个单位，在向左移动2个单位，有球掉在地上的弹性效果
	
    CCRect getScissorFrame();
...
public:
	...
	void setSelectedArrayByIndex(int num, bool isSelected);
    ...
    void moveCellByDistance(int startCellIndex, float moveDistance,bool isMoveDown = true,float preDownDis = 0);
    void setCellByDistance(int startCellIndex, float moveDistance,bool isMoveDown = true,float preDownDis=0);
	
    void moveBodyToPosition1(CCPoint pos);
    void endMove();
    void cellmove();
  
	
private:
	//根据indexPath复用cell，仅供自己用
	SNSTableViewCell* getAndReuseCellWithIndexPath(SNSIndexPath* indexPath);
