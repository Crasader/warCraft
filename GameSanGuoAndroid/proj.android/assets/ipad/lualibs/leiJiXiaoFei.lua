...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityLeiJiXiaoFei.plist" , 1 , 191)
	...
	local function createTemplateLayer(count , baseTag, buttonTag)--此时baseTag = 201 , count=4
		...
		--昨上方描述文本
		local labelDes = CCLabelTTF:create("累计消费500元可获得" , FONT_PANGWA , 24)
		...
		for i = 1 , count do
			...
			local function onTouchBegan(x, y)
				if rect:containsPoint(tempLayer:convertToNodeSpace(ccp(x,y)))  and tempLayer:getParent():isVisible() then
					--spriteBorder:setColor(ccc3(160,160,160))
					isTouchMoved = false
					beganPosY = y
					return true
				else
		...
		--描述文本右边  数量比例。
		local labelNum = CCLabelTTF:create("5000/5000" , FONT_PENGYOU , 24)
	...
	local function createRetLayer(baseTag)
        ...
        tempLayer_3:setPosition(visibleSize.width/2 , tempLayer_3:getContentSize().height * 0.5)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityLeiJiXiaoFei.png")
        ...
        retLayer:setContentSize( CCSize:new_local( visibleSize.width , spriteBg:getPositionY() + spriteBg:getContentSize().height * 0.5 - tempLayer_3:getPositionY()+ tempLayer_3:getContentSize().height * 0.5 + 15 ) )
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654343)
end
