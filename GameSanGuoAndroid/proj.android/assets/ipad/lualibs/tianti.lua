...
local function main()
    ...
    ResourceManager:sharedInstance():bindTexture("sanguobigpic/tianti.plist", 1, 191)
...
-------顶图
    local sprite2 = CCSprite:createWithSpriteFrameName("tianti.png")
    sprite2:setPosition(ccpAdd(ccp(spriteqp:getPosition()),ccp(0,sprite2:getContentSize().height/2+spriteqp:getContentSize().height/2)))
    layerFarm:addChild(sprite2)
    
-------底框字
    local labelinfo = CCLabelTTF:create("全民獎勵:活動截止時,累計獲得天梯聲望高于           的主公可獲得",FONT_BOXINFO,20)
...
-----level
    ...
    labellevel:setPosition(ccpAdd(ccp(labelinfo:getPosition()),ccp(133,0)))
...
-----三个国家+天梯数
    ...
    local labeljh = CCLabelTTF:create("活動期間累計獲得天梯聲望:",FONT_BOXINFO,22)
...
-------------天\时\分\秒
    ...
    day:setPosition(ccpAdd(ccp(sprite2:getPosition()),ccp(-53,220)))
...
------------大按钮
    ...
    btn6:setPosition(ccpAdd(ccp(sprite2:getPosition()),ccp(0,-17)))
    btn6:setTag(300)
    layerFarm:addChild(btn6)

-----------第二三名
    local btn4 =  createLayerButton4()
    btn4:setPosition(ccpAdd(ccp(btn6:getPosition()),ccp(-190,-200)))
    btn4:setTag(301)
    layerFarm:addChild(btn4)
    
    local btn5 =  createLayerButton5()
    btn5:setPosition(ccpAdd(ccp(btn6:getPosition()),ccp(190,-200)))
    btn5:setTag(302)
    ...
    sceneGame:addChild(createLayerFarm(),1,654332)
