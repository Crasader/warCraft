...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activity_qingming.plist" , 1 , 191)
	...
	local function createRetLayer(baseTag)
		...
		local orign = CCDirector:sharedDirector():getVisibleOrigin()
	     ...
	     local sumItem  = 4 --总共有几个条目 , 必须大于等于2
        --创建第一个条,最下面的那个
		local tempLayer = createTemplateLayer(4 , 1+(100 * sumItem), 40 + (100 * sumItem))
        tempLayer:ignoreAnchorPointForPosition(false)
        tempLayer:setAnchorPoint(ccp(0.5,0.5))
        tempLayer:setPosition(visibleSize.width/2 , tempLayer:getContentSize().height * 0.5)
        retLayer:addChild(tempLayer)
        
        --循环创建sumItme - 1 条目
        for i = 1, sumItem - 1 do

	        local tempLayer__ = createTemplateLayer(4 , 1+(100 * (sumItem - i  )),40 + (100 * (sumItem - i )))
			tempLayer__:ignoreAnchorPointForPosition(false)
			tempLayer__:setAnchorPoint(ccp(0.5,0.5))
			tempLayer__:setPosition(ccp(visibleSize.width/2 , tempLayer__:getContentSize().height * 0.5 + (tempLayer__:getContentSize().height +15 )*(i)) )
			retLayer:addChild(tempLayer__)

        end

		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activity_qingming_bg.png")
		spriteBg:setAnchorPoint(ccp(0.5,0.5))
        spriteBg:setPosition( visibleSize.width * 0.5 , (tempLayer:getContentSize().height +15)* sumItem + spriteBg:getContentSize().height * 0.5 )
		...
		--第二个label  活动描述
		local labelActivityDes = SGCCLabelTTF:create("活动期间，累计登陆到指定的天数,可以领取对应的奖励" , FONT_PANGWA , 22)
        ...
        retLayer:setContentSize( CCSize:new_local( visibleSize.width , spriteBg:getPositionY() + spriteBg:getContentSize().height * 0.5 - tempLayer:getPositionY()+ tempLayer:getContentSize().height * 0.5 + 15 ) )
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654369)
end
