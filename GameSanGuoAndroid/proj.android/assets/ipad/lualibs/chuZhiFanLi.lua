...
local function main()
	...
	--ToDo:在这里绑定资源
	ResourceManager:sharedInstance():bindTexture("sanguobigpic/activityDanBiChuZhi.plist" , 1 , 191)
	...
	local function createTemplateLayer(count , baseTag, buttonTag)--此时baseTag = 201 , count=4
		...
		--昨上方描述文本
		local labelDes = CCLabelTTF:create("单笔充值500元可获得" , FONT_PANGWA , 24)
	...
	local function createRetLayer(baseTag)
		...
		--该活动的背景图片
		local spriteBg = CCSprite:createWithSpriteFrameName("activityDanBiChuZhi.png")
	...
	runningScene:addChild(createRetLayer(1) , 1  ,654341)
end
